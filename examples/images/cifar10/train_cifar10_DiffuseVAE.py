...
flags.DEFINE_string("output_dir", "./results/d", help="output_directory")
# UNet
...
def train(argv):
    ...
    with trange(FLAGS.total_steps, dynamic_ncols=True) as pbar:
        for step in pbar:
            ...
            x0_tile = x0 + 0.01 * torch.randn_like(x0) 
            ...
            if FLAGS.save_step > 0 and step % FLAGS.save_step == 0:
                generate_samples_ext(x0_tile[:100],y1[:100],net_model, FLAGS.parallel, savedir, step, net_="normal", guidance=True)
                generate_samples_ext(x0_tile[:100],y1[:100],ema_model, FLAGS.parallel, savedir, step, net_="ema", guidance=True)
                torch.save(
                    {
