...
flags.DEFINE_integer("step", 300000, help="Training step of the checkpoint to load")
...
flags.DEFINE_string("save_path", "./generated_images_z.npz", help="Path to save the generated npz file")
...
PATH2 = f"{FLAGS.input_dir}/xT_yotcfm/otcfm_cifar10_weights_step_300000.pt"
...
state_dict2 = checkpoint2["ema_model"]
try:
    new_net2.load_state_dict(state_dict2)
except RuntimeError:
    from collections import OrderedDict

    new_state_dict2 = OrderedDict()
    for k, v in state_dict2.items():
        new_state_dict2[k[7:]] = v
    new_net2.load_state_dict(new_state_dict2)
new_net2.eval()
...
def gen_batch(batch_size):
    ...
    with torch.no_grad():
        ...
        with torch.autocast("cuda"):
            ...
            vf1 = lambda t, x, args=None: new_net(t, x, y)
            vf2 = lambda t, x, args=None: new_net2(t, x, y)
            node = NeuralODE(vf1, solver=FLAGS.integration_method)
            node2 = NeuralODE(vf2, solver=FLAGS.integration_method)
            t_span2 = torch.linspace(0, 1, 2, device=device)
            traj = node2.trajectory(x, t_span=t_span2)
            xT = traj[-1]
            #a = 0.2  # noise level
            #xT = np.sqrt(1-a**2) * xT + a * torch.randn_like(xT)
            if FLAGS.integration_method == "euler":
                print("Use method: ", FLAGS.integration_method)
                t_span = torch.linspace(0, 1, FLAGS.integration_steps + 1, device=device)
                traj = node.trajectory(xT, t_span=t_span)
                #traj = node2.trajectory(traj[-1], t_span=t_span2)
            else:
                print("Use method: ", FLAGS.integration_method)
                t_span = torch.linspace(0, 1, 2, device=device)
                #t_span2 = torch.linspace(0, 1, 2, device=device)
                traj = odeint(
                    vf1, xT, t_span, rtol=FLAGS.tol, atol=FLAGS.tol, method=FLAGS.integration_method
                )
