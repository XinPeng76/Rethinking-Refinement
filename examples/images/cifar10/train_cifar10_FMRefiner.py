...
flags.DEFINE_string("output_dir", "./results/i", help="output_directory")
# UNet
...
def train(argv):
    ...
    with trange(FLAGS.total_steps, dynamic_ncols=True) as pbar:
        for step in pbar:
            ...
            y = batch[1].to(device)
            a = 0.1*torch.rand(x1.size(0), 1, 1, 1).to(device)
            x0_tile = x1 + a * torch.randn_like(x1) 
            #t, xt, ut = FM.sample_location_and_conditional_flow(x0, x1)
            with torch.autocast("cuda", dtype=torch.bfloat16):
                t, xt, ut, _, y1 = FM.guided_sample_location_and_conditional_flow(x0_tile, x1, y1=y)
                t_expand = t.view(-1, 1, 1, 1)
                xT = 0.05 * torch.randn_like(x1)
                xt = xt + (1-t_expand)*t_expand*xT
                ut = ut + (1-2*t_expand)*xT
            ...
            if FLAGS.save_step > 0 and step % FLAGS.save_step == 0:
                generate_samples_ext(dataset0[:100],labels0[:100],net_model, FLAGS.parallel, savedir, step, net_="normal", guidance=True)
                generate_samples_ext(dataset0[:100],labels0[:100],ema_model, FLAGS.parallel, savedir, step, net_="ema", guidance=True)
                torch.save(
                    {
